...
def getData():
    ...
    return (data, rows, cols, images, filtersize, filters, testimgdata)
...
def ReLU(convol):
    ...
    for img in range(len(convol)):
        ...
        print('** Name of image: ' + str(convol[img][0]) + ' **')
...
def pooling(ReLU, poolFiltSize, stride, option):
    ...
    for img in range(len(ReLU)):
        ...
        print('** Name of image: ' + str(ReLU[img][0]) + ' **')
...
def makeFilters(data, rows, cols, images, filtersize, filters):
    # generate filters
    flts = [] # filter array
    i = 0
    while i < filters:
...
# CNN
def CNN(data, rows, cols, images, filtersize, filters, flts):
    ...
    # convolution 0
    print('******** CONVOLUTION 0 RESULT ********')
    convol0 = convolution(data, rows, cols, images, filtersize, filters, flts)
    ...
    newFilter = [[[1, -1], [-1, 1]]]
    convol1 = convolution(pooling0Data, math.floor((rows-1)/2), math.floor((cols-1)/2), images*filters, 2, 1, newFilter)
...
(data, rows, cols, images, filtersize, filters, testimgdata) = getData()

print('######## making CNN array ########')
print('')

# make filters
flts = makeFilters(data, rows, cols, images, filtersize, filters)
# CNN using the filter - data image
resultArrays = CNN(data, rows, cols, images, filtersize, filters, flts)
...
testArray = CNN([testimgdata], rows, cols, 1, filtersize, filters, flts)
...
for i in range(images):
    ...
    print('sumProduct of image ' + data[i][0] + ': ' + str(round(sumProduct, 6)))
    if sumProduct > maxVal:
        maxVal = sumProduct
        maxIndex = i

print('final prediction of ' + str(testimgdata[0]) + ': ' + data[maxIndex][0])
