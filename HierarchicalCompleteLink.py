...
# main: Hierarchical Clustering (Complete Link)
def Hierarchical(data, K, M):
    ...
    while 1:
        ...
        # 6. calculate sim(AB, X) = min(sim(A, X), sim(B, X)) for each member X
        #    -> using distance(AB, X) = max(distance(A, X), distance(B, X))
        #    -> update matrix
        last_i = 0
        for i in range(NC-1):
            if last_i == merge1: last_i += 1
            if last_i == merge2: last_i += 1
            maxdist = max(tm[merge1][last_i], tm[merge2][last_i])
            matrix[NC-1][i] = maxdist
            matrix[i][NC-1] = maxdist
            last_i += 1
