...
# main: Hierarchical Clustering (Group Average Link)
    ...
    while 1:
        ...
        for i in range(NC):
            for j in range(NC):
                tm[i][j] = matrix[i][j]
        tc = []
        for i in range(NC): tc.append(clusters[i])
        ...
        # 6. calculate sim(AB, X) = group-avg(sim(A, X), sim(B, X)) for each member X
        #    -> using distance(AB, X) = group-avg(distance(A, X), distance(B, X))
        #    -> update matrix
        last_i = 0
        for i in range(NC-1):
            ...
            cA = len(tc[merge1]) # count of cluster A
            cB = len(tc[merge2]) # count of cluster B
            cAB = cA + cB # total count of cluster A and B
            
            avgdist = cA * tm[merge1][last_i] / cAB + cB * tm[merge2][last_i] / cAB
            matrix[NC-1][i] = avgdist
            matrix[i][NC-1] = avgdist
            last_i += 1
